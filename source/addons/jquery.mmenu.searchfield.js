...
(function( $ ) {
	...
	$[ _PLUGIN_ ].prototype[ '_addon_' + _ADDON_ ] = function()
	{
		...
		_e.add( 'search reset keyup change update' );
		...
		if ( opts.search )
		{
			...
			if ( $input.length )
			{
				var $labels = $('ul.' + _c.list + '> li.' + _c.label, this.$menu),
					$items 	= $('ul.' + _c.list + '> li', this.$menu)
						.not( '.' + _c.subtitle )
						.not( '.' + _c.label )
						.not( '.' + _c.noresults );
				...
				this.$menu
					...
					.on( _e.search,
						function( e, query )
						{
							...
							$( $('.' + _c.panel, that.$menu).get().reverse() ).each(
								function()
								{
									var $t = $(this),
										$p = $t.data( _d.parent );
							...
							//	Update counters
							$('em.' + _c.counter, that.$menu).trigger( _e.update );
						}
					);
			}
		}
